...
# this comes from optimising lbd for all other conditions in plot_scan_results_xval.py on scan_JUWELS_47
...
for i in range(2):
    for j in range(2):
        for n in range(2):
            for o in range(2):
                id1 = lbd_id1[((((((i*2+j)*2+k)*2+l)*2+m)*2+n)*2+o)]
                fname = bname1+str(id1)+".json"
                print(fname)
                with open(fname,"r") as f:
                    p0= json.load(f)
                for s in range(8):
                    id0 = (((i*2+j)*2+n)*2+o)*8+s
                    print(id0)
                    p = p0
                    p["ORIG_NAME"] = fname
                    ...
                    p["OUT_DIR"] = "scan_JUWELS_48b/"
                    p["NAME"] = "J48b_scan_"+str(id0)
