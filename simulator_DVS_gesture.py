...
p["TRIAL_MS"]= 18000.0
p["N_MAX_SPIKE"]= 3000    # make buffers for maximally 400 spikes (200 in a 30 ms trial) - should be safe
p["N_BATCH"]= 32
p["N_TRAIN"]= 1045 # 55000
p["N_VALIDATE"]= 32
p["N_EPOCH"]= 100
p["SHUFFLE"]= True
...
class SMNIST_model:
    ...
    def __init__(self, p):
        ...
        if p["REDUCED_CLASSES"] is not None and len(p["REDUCED_CLASSES"]) > 0:
            self.X_train_orig, self.Y_train_orig = self.reduce_classes(self.X_train_orig, self.Y_train_orig, p["REDUCED_CLASSES"])
            self.X_test_orig, self.Y_test_orig, self.reduce_classes(self.X_test_orig, self.Y_test_orig, p["REDUCED_CLASSES"])


    def plot_examples(self,digit,nsample,phase):
        if phase == "train":
            X= self.X_train_orig
            Y= self.Y_train_orig
        else:
        ...
        X= np.array(X)
        xdim= int(np.ceil(np.sqrt(nsample)))
        ydim= nsample // xdim
        if xdim*ydim < nsample:
            ydim+= 1
        fig, ax= plt.subplots(ydim,xdim,sharex= True, sharey= True)
        for y in range(ydim):
            print(Y == digit)
            print(X)
            td= X[np.where(Y == digit)]
            td= td[y*xdim:(y+1)*xdim]
    ...
    def load_data(self, p, shuffle= True):
        if p["TRAIN_DATA_SEED"] is not None:
            self.datarng= np.random.default_rng(p["TRAIN_DATA_SEED"])
        else:
            self.datarng= np.random.default_rng()        
    ...
    # split available training data into a training and validation dataset at random
    def split_random(self, X, Y, p, shuffle= True):
        ...
        newX_t= [ X[i] for i in train_idx]
        newX_e= [ X[i] for i in eval_idx]
        newY_t= Y[train_idx]
        newY_e= Y[eval_idx]
        print(len(newX_t))
        print(len(newX_e))
    ...
    def train(self, p):
        ...
        if p["N_VALIDATE"] > 0:
            X_train, Y_train, X_eval, Y_eval= self.split_random(self.X_train_orig, self.Y_train_orig, p)
        else:
    ...
    def cross_validate(self, p):
        ...
        for i in speakers:
            ...
            if p["BUILD"]:
                self.model.build()
            self.model.load(num_recording_timesteps= p["SPK_REC_STEPS"])
            X_train, Y_train, X_eval, Y_eval= self.split_random(self.X_train_orig, self.Y_train_orig)
