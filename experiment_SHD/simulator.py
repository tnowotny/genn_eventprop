...
class mnist_model:
    ...
    def define_model(self, p, shuffle):
        ...
        if p["DATASET"] == "SHD":
            if p["LOSS_TYPE"] == "max":
                self.output_reset= self.model.add_custom_update("output_reset","neuronReset", EVP_neuron_reset_output_SHD_first_spike, output_reset_params, {}, output_var_refs)
            if p["LOSS_TYPE"] == "sum":
            ...
            if p["LOSS_TYPE"] == "avg_xentropy":
                self.output_reset= self.model.add_custom_update("output_reset","neuronReset", EVP_neuron_reset_output_SHD_avg_xentropy, output_reset_params, {}, output_var_refs)
