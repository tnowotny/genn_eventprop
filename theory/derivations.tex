...
\end{itemize}
  ...
  \end{align}
  and that one can deduce jumps for $\lambda_V$ at $t_{\text{max}}$ that are equivalent to the jumps normally encountered at spike times. If we follow that logic, we need to jump $\lambda_V$ by $\frac{\partial l_{\text{m}}(t_{\text{max}}^{j})}{\partial t_{\text{max}}^{k}}$ at the times $t_{\text{max}}^{k}$, where
  \begin{align}
  ...
  However, this does not generate a functional form as in {\blue
    (1)} on first sight. It is equally difficult to argue with
  $V_{\text{max}} = V(t_{\text{max}})$ as the $t_{\text{max}}$ are not
  ...
  The cross-entropy loss functions based on maximal voltages and
  average voltages above do not really seem to work well in the
...
which it is not. I am not sure this is mathematically correct ... though the MNIST example appeared to learn.
  ...
  As an alternative to the loss functions (\ref{maxloss}) and (\ref{sumloss}) which are technically not compliant to the eventProp algorithm I decided to work with the average cross-entropy loss
  \begin{align}
    {\cal L}= \int_0^T  - \frac{1}{N_{\text{batch}}} \sum_{i=1}^{N_{\text{batch}}} \log \frac{\exp(V_{l(i)} (t)}{\sum_{k=1}^{N_{\text{class}}} \exp(V_k(t))}
  \end{align}
...
{"NAME": "test43", "DATASET": "SHD", "DEBUG": false, "DEBUG_HIDDEN_N": true, "OUT_DIR": ".", "DT_MS": 1, "BUILD": true, "TIMING": true, "TRAIN_DATA_SEED": 372, "TEST_DATA_SEED": 814, "MODEL_SEED": 135, "TRIAL_MS": 1400, "N_MAX_SPIKE": 1500, "N_BATCH": 32, "SUPER_BATCH": 1, "N_TRAIN": 7900, "N_VALIDATE": 512, "N_EPOCH": 10, "SHUFFLE": true, "N_TEST": 10000, "NUM_HIDDEN": 256, "RECURRENT": false, "TAU_SYN": 5.0, "TAU_MEM": 20.0, "V_THRESH": 1.0, "V_RESET": 0.0, "INPUT_HIDDEN_MEAN": 0.02, "INPUT_HIDDEN_STD": 0.01, "HIDDEN_OUTPUT_MEAN": 0.0, "HIDDEN_OUTPUT_STD": 0.3, "HIDDEN_HIDDEN_MEAN": 0.0, "HIDDEN_HIDDEN_STD": 0.02, "PDROP_INPUT": 0.1, "PDROP_HIDDEN": 0.0, "REG_TYPE": "simple", "LBD_UPPER": 0, "LBD_LOWER": 0, "NU_UPPER": 15, "NU_LOWER": 5, "RHO_UPPER": 10000.0, "GLB_UPPER": 1e-08, "ETA": 0.002, "ADAM_BETA1": 0.9, "ADAM_BETA2": 0.999, "ADAM_EPS": 1e-08, "ETA_DECAY": 1.0, "ETA_FIDDELING": false, "ETA_REDUCE": 0.5, "ETA_REDUCE_PERIOD": 50, "W_OUTPUT_EPOCH_TRIAL": [], "SPK_REC_STEPS": 1400, "REC_SPIKES_EPOCH_TRIAL": [[0, 0], [0, 1], [0, 2], [0, 3], [9, 0], [9, 1], [9, 2], [9, 3], [59, 0], [59, 1], [59, 2], [59, 3], [599, 0], [599, 1], [599, 2], [599, 3], [999, 0], [999, 1], [999, 2], [999, 3]], "REC_SPIKES": ["input", "hidden"], "REC_NEURONS_EPOCH_TRIAL": [[0, 0], [0, 1], [0, 2], [0, 3], [9, 0], [9, 1], [9, 2], [9, 3], [59, 0], [59, 1], [59, 2], [59, 3], [599, 0], [599, 1], [599, 2], [599, 3], [999, 0], [999, 1], [999, 2], [999, 3]], "REC_NEURONS": [["output", "V"], ["output", "sum_V"], ["output", "lambda_V"], ["output", "lambda_I"], ["hidden", "lambda_V"]], "REC_SYNAPSES_EPOCH_TRIAL": [], "REC_SYNAPSES": [], "WRITE_TO_DISK": true, "LOAD_LAST": false, "LOSS_TYPE": "avg_xentropy", "EVALUATION": "speaker", "CUDA_VISIBLE_DEVICES": false, "AVG_SNSUM": true, "REDUCED_CLASSES": [0], "W_REPORT_INTERVAL": 11000, "REWIRE_SILENT": false, "AUGMENTATION": {"random_shift": 20.0, "random_dilate": [0.9, 1.1], "ID_jitter": 5.0}}
  \end{lstlisting}    
  
  This runs the input 0 only and with the normal trial duration of 1400 ms. We observe that the network unlearns if anything and the number of spikes in the hidden layer goes to 0.

  If we, however, use the same set of parameters but
    \begin{lstlisting}[language=json]
      {"TRIAL_MS": 400}
  \end{lstlisting}    
    then the network learns the task perfectly, with rapidly decreasing loss. The number of spikes in the hidden layer is increasing if anything. This illustrates how (I think) the silent time at the end of the trial (which carries considerable loss as all output voltages are equal 0) is affecting the gradient in such a way that hidden neurons' spikes are moved to later times but then inadvertantly deleted and the hidden layer falls silent and eventually the network fails completely.

    When we switch on regularisation from below:
     \begin{lstlisting}[language=json]
      {"TRIAL_MS": 1400, p["LBD_LOWER"], 2e-8}
  \end{lstlisting}    
     then activity in the hidden layer is recovered but learning is still not happening!
 I believe this is because it is not only a general drive to silence hidden layer neurons but (unintuitively) in particular a drive to silence the neurons that contribute to depolarising the correct output neuron. Hence, the (expected) potentiation of active neurons synapses towards the correct otuput neuron is negated by specific weakening of the said active neurons' inputs. I have explicitly observed that hidden neurons with strong positive connections to the correct output neuron appear to emit fewer spikes than those with smaller positive or negative weights ... 

FIGURE HERE SHOWING spike raster and weights barplot.
 
     A detailed understanding of this effect in the context of the full system with 20 classes and batched training on many examples is difficult but the simplistic experiments described above suggest that the local exact gradient for the average cross-entropy loss inadvertently tends to switch off the hidden neurons best placed to represent a given class and so negates global optimisation (for the normal 1400 ms duration). It is worth noting in this context that (I believe) the local gradient ``cannot know'' about spike deletion once the input gets too weak. If moving a spike time to a later point in time is reducing the loss, following the local gradient will lead to reduced weights into the neuron in question (which will delay the spike but might also delete it!).

     Running the network with a trial time of 400 ms,
     \begin{lstlisting}[language=json]
{"NAME": "test43", "DATASET": "SHD", "DEBUG": false, "DEBUG_HIDDEN_N": true, "OUT_DIR": ".", "DT_MS": 1, "BUILD": true, "TIMING": true, "TRAIN_DATA_SEED": 372, "TEST_DATA_SEED": 814, "MODEL_SEED": 135, "TRIAL_MS": 400, "N_MAX_SPIKE": 1500, "N_BATCH": 32, "SUPER_BATCH": 1, "N_TRAIN": 7900, "N_VALIDATE": 512, "N_EPOCH": 200, "SHUFFLE": true, "N_TEST": 10000, "NUM_HIDDEN": 256, "RECURRENT": false, "TAU_SYN": 5.0, "TAU_MEM": 20.0, "V_THRESH": 1.0, "V_RESET": 0.0, "INPUT_HIDDEN_MEAN": 0.02, "INPUT_HIDDEN_STD": 0.01, "HIDDEN_OUTPUT_MEAN": 0.0, "HIDDEN_OUTPUT_STD": 0.3, "HIDDEN_HIDDEN_MEAN": 0.0, "HIDDEN_HIDDEN_STD": 0.02, "PDROP_INPUT": 0.1, "PDROP_HIDDEN": 0.0, "REG_TYPE": "simple", "LBD_UPPER": 0, "LBD_LOWER": 2e-08, "NU_UPPER": 15, "NU_LOWER": 5, "RHO_UPPER": 10000.0, "GLB_UPPER": 1e-08, "ETA": 0.002, "ADAM_BETA1": 0.9, "ADAM_BETA2": 0.999, "ADAM_EPS": 1e-08, "ETA_DECAY": 1.0, "ETA_FIDDELING": false, "ETA_REDUCE": 0.5, "ETA_REDUCE_PERIOD": 50, "W_OUTPUT_EPOCH_TRIAL": [], "SPK_REC_STEPS": 400, "REC_SPIKES_EPOCH_TRIAL": [[0, 0], [0, 1], [0, 2], [0, 3], [9, 0], [9, 1], [9, 2], [9, 3], [59, 0], [59, 1], [59, 2], [59, 3], [599, 0], [599, 1], [599, 2], [599, 3], [999, 0], [999, 1], [999, 2], [999, 3]], "REC_SPIKES": ["input", "hidden"], "REC_NEURONS_EPOCH_TRIAL": [[0, 0], [0, 1], [0, 2], [0, 3], [9, 0], [9, 1], [9, 2], [9, 3], [59, 0], [59, 1], [59, 2], [59, 3], [599, 0], [599, 1], [599, 2], [599, 3], [999, 0], [999, 1], [999, 2], [999, 3]], "REC_NEURONS": [["output", "V"], ["output", "sum_V"], ["output", "lambda_V"], ["output", "lambda_I"], ["hidden", "lambda_V"]], "REC_SYNAPSES_EPOCH_TRIAL": [], "REC_SYNAPSES": [], "WRITE_TO_DISK": true, "LOAD_LAST": false, "LOSS_TYPE": "avg_xentropy", "EVALUATION": "speaker", "CUDA_VISIBLE_DEVICES": false, "AVG_SNSUM": true, "REDUCED_CLASSES": null, "W_REPORT_INTERVAL": 11000, "REWIRE_SILENT": false, "AUGMENTATION": {"random_shift": 20.0, "random_dilate": [0.9, 1.1], "ID_jitter": 5.0}}
  \end{lstlisting}    

     we observe some learning initially, but lack-luster ... around 20\% successful (both training and evaluation) and eventually it also fails (0 activity in the hidden layer). For even stronger regularisation $\lambda_{\text{upper}}= 2e-8$, $\lambda_{\text{lower}} = 2e-7$, the learning is even less effective (around 10\% correct ...) hum ... saw training loss == inf - something weird is going on still?
     
     \section{Some notes about the incorrect ``constant cross-entropy'' attempt}
     I did notice previously that regularisation needed to be a lot weaker in the hidden layer with this model and eventprop as opposed to Jamie's e-prop network. Subsequently it became clear that due to the constant cross-entropy term that was driving $\lambda_V$ in the output neurons, the transmitted $\lambda_V-\lmabda_I$ was extremely small (just stemming from $\lambda_I$ lagging in the exponential rise/fall to (driven) steady state from starting at $0$. Weight changes in the hidden layer were therefore most delicate and easily destroyed by too strong regularisation. This small dirve also explains the slow learning observed compared to e-prop, Jamie typically trains 50 epochs and has convergence easily while here, 200 or more were advised (and that already included fairly high global learning rate).

     So, in summary, the incorrect constant cross-entropy approach was clearly not ideal and is not sensible for this reason alone. It also appears quite coarse as no temoral dynamics of the neurons is actually considered. A more fine-grained loss function should be more successful (though with the full toolbox of augmentation etc the performance was not horrible - though it took a long while to achieve it).
